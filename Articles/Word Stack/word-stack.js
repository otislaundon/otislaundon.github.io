...
function rand_block() {return new block(rand_int(7));}
...
function is_word(maybe_word) {
    return wordlist.includes(maybe_word);
}

function select_square(x, y, val) {
    let selection_info = "";
    let square_score = 0;
...
function clear_words_from_tiles() {
...
function find_words() {
    ...
    for(let y0 = 0; y0 < bheight; y0++)
    {
        ...
        for(let len = Math.min(bheight - y0, bwidth - x0); len >= 3; len --){ // maybe change this to be ++ isntead of --
            let str_extract = extract_str(x0, y0, 1, 1, len);
            if(is_word(str_extract))
...
function find_words_in_row(i) {
    let words_in_row = [];
    for(let x0 = 0; x0 < bwidth; x0++)
    {
        // find largest horizontal word at (x0, i)
        for(let len = bwidth - x0; len >= 3; len --){
            let str_extract = extract_str(x0, i, 1, 0, len);
            if(is_word(str_extract))
...
function collect_at_square(x, y) {
...
function extract_str(x, y, dx, dy, len) {
    let res = "";
    for(let i = 0; i < len; i++)
    {
        if(board[y][x].letter == "")
...
// RENDERING FUNCTIONS - functions that directly modify the html and change the appearance of the site

function create_board_display(n, m) {
...
function input_spinr() {
    if(block_cur == null || game_ended) return false;
    return block_cur.transform_checked(0,0,1);
}

function input_spinl() {
    if(block_cur == null || game_ended) return false;
    return block_cur.transform_checked(0,0,-1);
}
